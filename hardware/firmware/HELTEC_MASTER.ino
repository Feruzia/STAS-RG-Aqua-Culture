...
unsigned long lastSDLog = 0;  // Timer khusus untuk SD Card
...
void setup() {
  ...
  if (!SD.begin(SD_CS)) {
    ...
    if (!SD.exists("/dataset_toksisitas.csv")) {
      File dataFile = SD.open("/dataset_toksisitas.csv", FILE_WRITE);
      if (dataFile) {
        dataFile.println("Tanggal,Waktu,Tipe,Suhu_C,pH,DO_mgL,Turb_NTU,EC_uS,TDSe_ppm,TDSs_ppm,ORP_mV");
        dataFile.close();
        Serial.println("[OK] File dataset_toksisitas.csv dibuat.");
      }
    }
  }
